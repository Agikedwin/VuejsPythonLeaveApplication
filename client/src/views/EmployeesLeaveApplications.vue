      ...
      <template slot="items" slot-scope="props">
          ...
          <v-tooltip bottom>
            <template v-slot:activator="{ on }">
              <v-btn flat fab v-on="on">
                <v-icon dark class="teal--text" >email</v-icon>
              </v-btn>
            </template>
            <span>{{props.item.application_note}}</span>
          </v-tooltip>
        </td>
        ...
        <div v-if="props.item.approved==1">
          <td class="text-xs-left">
            <v-tooltip bottom>
              <template v-slot:activator="{ on }">
                <v-btn flat fab color="success" v-on="on">
...
export default {
  ...
  methods: {
    async fetchLeaveApplication() {
      await api.getEntries("getAllLeaveApplications").then(empList => {
        setTimeout(duration => {
    ...
    async addRecord(paylaod) {
      await api.addEntry("addApprovals", paylaod).then(data => {
